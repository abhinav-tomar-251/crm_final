...
const sendViaApi = async ({ email, name, targetLocation }) => {
  ...
  const botmartz_app_email = 'noreply@botmartzapp.com';
  const botmartz_app_company_email = settings['botmartz_app_company_email'];
  ...
  const { data } = await resend.emails.send({
    ...
    reply_to: botmartz_app_company_email,
    attachments: [
      {
        filename: 'Quote.pdf',
